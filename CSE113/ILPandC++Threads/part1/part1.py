...
def homework_loop_sequential_source(chain_length, unroll_factor):
    ...
    for i in range(0,unroll_factor):
       chain.append("    float temp" + str(i) + " = b[i + " + str(i)+ "];")
       for j in range(0,chain_length):
            chain.append("    temp"+ str(i) +"+= "+ str(j+1)+".0f;")
       chain.append("   b[i + " + str(i) + "] = temp" + str(i) + ";")
    loop_close = "  }"
    function_close = "}"
...
def homework_loop_interleaved_source(chain_length, unroll_factor):
    ...
    for i in range(0,unroll_factor):
        chain2.append( "    float temp"  + str(i) + " = b[i+" + str(i) + "];")
        chain3.append("    b[i+" + str(i) + "] = temp" + str(i) + ";")
    for j in range(0,chain_length):
            for k in range(0,unroll_factor):
                chain.append("    temp"+ str(k) +"+= "+ str(j+1)+".0f;")

    loop_close = "  }"
    function_close = "}"
    return "\n".join([function, loop,"\n".join(chain2) ,"\n".join(chain), "\n".join(chain3), loop_close, function_close])