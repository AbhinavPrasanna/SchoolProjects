...
exprToString :: Expr -> String -- Writing the expected print statements as requested in the ReadME.
exprToString VarX                 = "x"
exprToString VarY                 = "y"
...
exprToString (Thresh e1 e2 e3 e4) = "(" ++ exprToString e1 ++ "<" ++ exprToString e2 ++ "?" ++ exprToString e3 ++ ":"
                                   ++ exprToString e4 ++ ")"
...
eval :: Double -> Double -> Expr -> Double --Adding haskell code to actually compute sin, cos, average, times, and thresh
--eval x y e = 
eval x y  VarX                                                = x
eval x y  VarY                                                = y 
...
eval x y (Thresh e1 e2 e3 e4) | (eval x y e1) < (eval x y e2) = (eval x y e3) --  if e2 is greater than e1 evaluted return e3
                              | otherwise                     = (eval x y e4) -- else return e4
...
build 0
  ...
  where
    r         = rand 10
build d 
   | (r == 1) = build(d - 1) --If r == 1 build the expression tree without new constructors
   | (r == 2) = Sine(build(d - 1)) -- else add sine constructor
   | (r == 3) = Cosine(build(d-1)) --else add cosine constructor
   | (r == 4) = Average(build(d-1)) (build(d-1)) --else add average constructor with two child expressions
   | (r == 5) = Times(build(d-1))(build(d-1)) --else add times constructor with 3 child expressions
   | (otherwise) = (Thresh(build(d-1))) (build(d-1)) (build(d-1)) (build(d-1)) --else add a thresh constructor with four child expressions
   where 
    r = rand 10
--------------------------------------------------------------------------------
-- | Best Image "Seeds" --------------------------------------------------------
...
-- grayscale
g1, g2, g3 :: (Int, Int) --Values I randomly tested on my local codespace to see if it worked.
g1 = (4, 55)
g2 = (6, 78)
g3 = (5, 103)
